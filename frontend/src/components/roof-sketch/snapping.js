// Pure snap-target selection for the Roof Sketch canvas (Node-testable). Tolerance is SCREEN-space and
// converted to model units by the caller (modelTol = snapPixels / view.k) so the effective snap zone is
// the same on screen at every zoom. Deterministic priority: existing vertex/endpoint > edge interior >
// free point. Never snaps to infinite-line extensions (projection stays on the actual segment).
import { projectPointToSegment } from "@roofspan/roof-sketch-core";

export function modelTolerance(snapPixels, viewK) {
  return snapPixels / (viewK || 1);
}

export function snapTarget(doc, point, modelTol, { excludeVertexId = null, eligibleEdge = () => true } = {}) {
