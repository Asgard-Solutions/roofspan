...
- **RoofSpan Office now installs like a normal Windows app**: a Desktop icon AND a Start Menu entry so it appears in Windows Search / All Apps. Because Office is a LOCAL browser-based UI (no standalone GUI window), a tiny windowless launcher exe opens the local UI in the default browser.
- **Launcher** (`windows/winbuild/office_launcher.py` -> `RoofSpanOffice.exe`, PyInstaller `roofspan-office-launcher.spec`, `console=False`): opens `http://127.0.0.1:8001/` (overridable via `ROOFSPAN_OFFICE_URL`). Not a service, not auto-started. Branded with committed `windows/installer/RoofSpanOffice.ico` (multi-res 16-256px, generated from `roofspan-appicon.png`).
- **WiX** (`RoofSpan.wxs`): new `App_Launcher` component packages `tools\RoofSpanOffice.exe` with two non-advertised shortcuts both named "RoofSpan Office" -> `DesktopFolder` (All-Users desktop, perMachine) + `StartMenuRoofSpan` (Start Menu -> Windows Search/All Apps). Shortcuts use the exe's embedded RoofSpan icon. Removed cleanly on uninstall.
- **Build wiring**: `targets.py` TOOL_TARGETS + `build_exes.ps1` toolSpecs build the launcher into staged `tools\`; `build.ps1` fail-fasts if `tools\RoofSpanOffice.exe` isn't staged.
- **Tests**: `windows/tests/test_installer_static.py` +2 (desktop/start-menu shortcuts launch Office; launcher built/packaged + .ico committed). Full windows suite **140/140 pass**; launcher URL resolution smoke-tested. **HUMAN REQUIRED**: native Windows rebuild + clean-PC install to visually confirm the Desktop icon and Start-Menu/search entry.