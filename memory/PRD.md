...
## Deterministic Roof-Framing Solver — Stages 0-4 + wiring DONE & VERIFIED (2026-06)
Replaced the old fan-out "pinwheel" heuristic for the topologies below with a real ridge-based framing solver. Full blueprint in `/app/memory/ROOF_FRAMING_SOLVER_PLAN.md`.
- **New shared module `packages/roof-sketch-core/frameRoof.js`** (`frameRoof(base, edgesIn, resolutions)`, exported via index.js). Builds the roof from the RIDGE outward as rectangular cores; deterministic, validated, no overlaps. Returns null to DEFER any topology it can't uniquely solve (caller falls back to the legacy `resolvePlacement` scaffold — no regression).
  - **`trySingleCore`** (Stage 1/3): one rectangular core = one ridge + 0..2 hip-end triangles. Handles simple gable, full standard hip, and half-hip. Equal-pitch (symmetric) required; ridge at mid-depth, hip insets split from the measured ridge length. Reproduces the standard-hip footprint exactly (apexes at W/2).
  - **`tryLRoof`** (Stage 2/4): two perpendicular equal-pitch, equal-width arms = 4 planes / 2 ridges + 1 hip + 1 valley. Produces a REAL convex HIP at the outer corner and a REAL concave VALLEY (shared miter edge) at the reentrant corner, ridges meeting at (W/2,W/2). This is the deterministic multi-core+valley case that directly replaces the pinwheel failure.
- **Wired (Stage 7)** into `generateSketchGeometry._layoutConnected` single-component branch: after `_tryStandardHip`, route to `frameRoof`; on a valid result return success/partial tagged with diagnostic `roof_framing_solved`; else fall through to the existing scaffold/resolvePlacement path. Simple archetypes (single/gable) untouched.
- **Verified:** NEW `test/frameRoof.node.test.js` 9 assertions (gable, standard hip footprint+apexes, L-roof hip+valley concavity, determinism, orchestrator end-to-end) — added to mobile `test:sketch`. ALL 12 core Node suites green; mobile `test:sketch` green; resolvePlacement 7/7 + generateSketchConnected 41/41 unregressed. Live-API integration `scripts/verify_lroof.js` → seeded L-roof returns high_confidence/ok/4 facets/1 hip/1 valley. **Office E2E testing_agent iteration_70: 100% (4/4)** — logged in, opened the seeded L-roof revision's Roof Sketch, Generate → High confidence, 4-plane L with hip+valley renders (no pinwheel/empty), Use Proposed adopts without error.
  - NOTE (not a bug): iteration_70 flagged per-facet SF deltas on the seed fixture — an artifact of arbitrarily-seeded `area_sqft` that didn't match the seeded dims. The solver correctly draws from Width/Length/pitch and preserves measured area; a dimension-consistent revision shows no delta.
- **STILL TODO to reach the full golden 8-plane roof (definition of done):** Stage 5 dormers (F5-F8 seated on host planes, overlap-on-host is a validator WARNING = acceptable); general multi-core (>2 cores, cross-gable/T, unequal-width L); asymmetric (unequal-pitch) cores; Stage 6 penetration placement/readiness polish. These currently DEFER to the resolve-placement scaffold (no pinwheel, but user resolves sides).
