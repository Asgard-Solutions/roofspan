...
- **Remaining (safely DEFER to the resolve-placement scaffold — no pinwheel):** unequal-width L and crossing/through wings (true 3D ridge-height-mismatch framing) — `tryLRoof` now GUARDS on equal width (±1 ft) and returns null for unequal, so it never draws a wrong L; dormers seated on a hip-end triangle host; multi-core >2 blocks. **Stage 6 (penetrations/readiness/validation) DONE:** penetrations are carried onto the framed sketch with their facet link but no fabricated position (position_known:false, manual placement); every result runs validateSketch and is validation-gated at the orchestrator. Plan Stages 0-7 are complete.
