...
function buildMapDiagnostic(fields = {}, now = () => new Date().toISOString()) {
  const {
    ...
    mapMountAttempted, mapMountSucceeded, areaCount, selectedAreaId,
    error,
  } = fields;
  ...
  return {
    ...
    map_mount_attempted: _bool(mapMountAttempted),
    map_mount_succeeded: _bool(mapMountSucceeded),
    area_count: typeof areaCount === "number" ? areaCount : null,
    selected_area_id: selectedAreaId != null ? String(selectedAreaId) : null,
    error_name: name,
...
function buildMapLoadDiagnostic(f = {}, now = () => new Date().toISOString()) {
  ...
  return {
    ...
    property_status: f.propertiesOk ? "ok" : "failed",
    ...
    area_count: typeof f.areaCount === "number" ? f.areaCount : null,
    selected_area_id: f.selectedAreaId != null ? String(f.selectedAreaId) : null,
    map_mount_attempted: f.mapMountAttempted === true,
    map_mount_succeeded: f.mapMountSucceeded === true,
    native_available: f.nativeAvailable === true,
    execution_environment: f.executionEnvironment != null ? String(f.executionEnvironment) : null,
