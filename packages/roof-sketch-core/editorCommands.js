"use strict";
// Pure, platform-neutral editor commands (CommonJS). Each returns the NEXT canonical sketch document
// (never mutates the input). Behavior is ported verbatim from the approved Office implementation
// (frontend/src/components/roof-sketch/commands.js). Geometry/validation/proposal math stays in the
// shared core modules (schema/geometry/topology/proposals) — this module never re-implements it.
const { normalizeSketchDocument } = require("./schema");
const { calibrateScale, distance, projectPointToSegment } = require("./geometry");
const { validateSketch } = require("./topology");

let _seq = 0;
const nid = (p) => `${p}_${Date.now().toString(36)}${(_seq++).toString(36)}${Math.random().toString(36).slice(2, 6)}`;
...
const vById = (doc, id) => (doc.vertices || []).find((v) => v.id === id) || null;
const eById = (doc, id) => (doc.edges || []).find((e) => e.id === id) || null;
const fById = (doc, id) => (doc.facets || []).find((f) => f.id === id) || null;

function addVertex(doc, x, y) {
  const d = clone(doc);
...
function moveVertex(doc, vertexId, x, y) {
  const d = clone(doc);
...
function moveVertexFinal(doc, vertexId, x, y) {
...
function deleteVertex(doc, vertexId) {
  let d = clone(doc);
...
function addEdge(doc, v1, v2, type = "unclassified") {
  if (v1 === v2) return doc;
...
function setEdgeType(doc, edgeId, type) {
  const d = clone(doc);
  d.edges = d.edges.map((e) => (e.id === edgeId ? { ...e, type } : e));
  return d;
}

function deleteEdge(doc, edgeId) {
  const d = clone(doc);
...
function splitEdge(doc, edgeId, x, y) {
...
function createFacet(doc, edgeIds, extra = {}) {
  const d = clone(doc);
...
function createManualFacet(doc, vertexIds, extra = {}) {
  const d = clone(doc);
...
function setScale(doc, { edgeId, realFeet, canvasDistance, method } = {}) {
...
function setProposalDecision(doc, { targetType, targetId, metric, decision, value } = {}) {
  const d = clone(doc);
...
function setDecisions(doc, decisions) {
  const d = clone(doc);
  d.proposal_decisions = [...(decisions || [])];
  return d;
}

function decisionFor(doc, targetType, targetId, metric) {
...
function isMeasurementFacetTaken(doc, measurementFacetId, exceptFacetId) {
...
function setFacetMeasurementLink(doc, facetId, measurementFacetId) {
...
function isMeasurementEdgeTaken(doc, measurementEdgeId, exceptEdgeId) {
...
function setEdgeMeasurementLink(doc, edgeId, measurementEdgeId) {
...
function setEditMode(doc, mode) {
  return normalizeSketchDocument({ ...clone(doc), edit_mode: mode });
}

// ---- topology-safe geometry operations (all pure; return {ok, doc, reason?}) ----
...
function validateMutation(before, after) {
  const count = (doc) => {
...
function splitEdgeSafe(doc, edgeId, x, y, { endpointTol = 1e-6 } = {}) {
...
function mergeVertices(doc, movingVertexId, targetVertexId) {
  if (doc.edit_mode !== "connected_graph") return { ok: false, reason: "connected_graph_required", doc };
...
function insertExistingVertexIntoEdge(doc, vertexId, edgeId, x, y, { endpointTol = 1e-6 } = {}) {
  if (doc.edit_mode !== "connected_graph") return { ok: false, reason: "connected_graph_required", doc };
  const e = eById(doc, edgeId);
  if (!e) return { ok: false, reason: "edge_not_found", doc };
...
function joinEdges(doc, aId, bId, { resultType } = {}) {
  if (doc.edit_mode !== "connected_graph") return { ok: false, reason: "connected_graph_required", doc };
