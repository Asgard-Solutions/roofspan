...
$line = @($text -split "`n" | Where-Object { $_ -match '^DATABASE_URL=' })
...
function Invoke-Probe([string]$Sql) {
    $result = & $psql -X -h 127.0.0.1 -p 5432 -U roofspan -d roofspan -w -v ON_ERROR_STOP=1 -tAc $Sql 2>&1
...
try {
    ...
    $wrong = [regex]::Replace($text, '(?m)^DATABASE_URL=.*$', 'DATABASE_URL=postgresql+asyncpg://roofspan:CI_WRONG_PASSWORD@127.0.0.1:5432/roofspan')
    try {
    ...
    Write-Host 'OK: real Burn/MSI legacy upgrade preserved data, config and PostgreSQL; Office backend is healthy.'
} finally {
    $env:PGPASSWORD = ''
